...
negative_text = df[df.sentiment == 'Negative']['final_text']
if len(negative_text) == 0:
    st.warning("  NO NEGATIVE TEXTS FOUND. PLEASE SEARCH FOR ANOTHER TWITTER HANDLE.", icon= "🚨")
else:
    ...
    with col1:
        try:
            ...
            st.warning(" VERY FEW NEGATIVE TEXTS", icon= "🚨")

    with col2:
        try:
            fig = px.bar(word_counts_df, y= 'word', x = 'count', title= "Most common negative words")
