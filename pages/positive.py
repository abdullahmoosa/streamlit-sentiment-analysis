...
positive_text = df[df.sentiment == 'Positive']['final_text']
if len(positive_text) == 0:
    st.warning("  NO POSITIVE TEXTS FOUND. PLEASE SEARCH FOR ANOTHER TWITTER HANDLE.", icon= "🚨")
else:
    ...
    words = get_words(positive_text)

    word_counts_df = get_words_df(positive_text, words)
    ...
    with col1:
        try:
            ...
            st.warning(" VERY FEW POSITIVE TEXTS", icon= "🚨")

    with col2:
        try:
            fig = px.bar(word_counts_df, y= 'word', x = 'count',)
            fig.update_layout( yaxis={'categoryorder':'total descending'}, title= "Most common positive words")
            st.plotly_chart(fig)
        except:
            st.warning(" VERY FEW POSITIVE TEXTS", icon= "🚨")
    ...
    try:
        ...
        st.markdown("### Word Cloud Plot of Positive sentiment")
        st.plotly_chart(fig)
